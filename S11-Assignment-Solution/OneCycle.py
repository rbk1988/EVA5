...
class OneCycle(object):
    ...
    def __init__(self, nb, max_lr, momentum_vals=(0.95, 0.85), prcnt= 10, div=10, use_cosine=False):
        ...
        self.div = div
        self.high_lr = max_lr
        self.low_lr = self.high_lr / self.div
        self.lr_step_size = self.high_lr - self.low_lr
        self.low_mom = momentum_vals[1]
        self.high_mom = momentum_vals[0]
        self.mom_step_size = self.high_mom - self.low_mom
    ...
    def calc_lr(self):
        if self.iteration <= self.step_len:
            m = self.lr_step_size / self.step_len
            c = self.low_lr
        else:
            m = -1*self.lr_step_size/(self.nb - self.step_len)
            c = (
                self.high_lr
                + self.step_len * self.lr_step_size / (self.nb - self.step_len)
            )

        lr = m * self.iteration + c
        self.lrs.append(lr)
        return lr

    def calc_mom(self):
        if self.iteration <= self.step_len:
            m = -1 * self.mom_step_size / self.step_len
            c = self.high_mom
        else:
            m = self.mom_step_size/(self.nb - self.step_len)
            c = self.low_mom - m * self.step_len

        mom = m * self.iteration + c
